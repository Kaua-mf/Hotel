...
import model.Modelo;
import model.Marca; // Adicionado

public class VeiculoDAO implements InterfaceDAO<Veiculo> {

    // --- CREATE (Insere um novo Veículo) ---
    @Override
    public void Create(Veiculo objeto) {
        // SQL: 8 colunas para os 8 '?'s que vamos preencher
        ...
        try {
            ...
            // Mapeamento das Chaves Estrangeiras (FKs)
            // Se o objeto for nulo, usamos setNull, pois as colunas aceitam NULL (INT)
            
            // MODELO_ID (Parâmetro 3)
            ...
            // MARCA_ID (Parâmetro 4) - FOCO PRINCIPAL DO SEU ERRO
            if (objeto.getMarca() != null && objeto.getMarca().getId() != 0) {
                pstm.setInt(4, objeto.getMarca().getId());
            } else {
                pstm.setNull(4, java.sql.Types.INTEGER); // Permite NULL
            }
            
            // FUNCIONARIO_ID (Parâmetro 5)
    ...
    public Veiculo Retrieve(int id) {
        // Ajuste no SELECT para incluir 'marca_id'
        ...
        try {
            ...
            if (rst.next()) {
                ...
                // Mapeamento de FKs (simplificado, apenas para não ser null)
                // O mapeamento real exigiria a chamada de outros DAOs (MarcaDAO, ModeloDAO)
                veiculo.setModelo((Modelo)null); // Simplificado
                veiculo.setMarca((Marca)null); // Simplificado
                veiculo.setFuncionario((Funcionario)null); // Simplificado
                veiculo.setFornecedor((Fornecedor)null); // Simplificado
                veiculo.setHospede((Hospede)null); // Simplificado
    ...
    public List<Veiculo> Retrieve(String atributo, String valor) {
        ...
        try {
            ...
            while(rst.next()) {
                ...
                // Mapeamento de FKs (simplificado)
    ...
    public void Update(Veiculo objeto) {
        // Ajuste no UPDATE para incluir 'marca_id'
        ...
        try {
            ...
            // Mapeamento das FKs (setando NULL se o objeto for null)
            ...
            // Mapeamento final
            pstm.setString(8, String.valueOf(objeto.getStatus()));
            pstm.setInt(9, objeto.getId()); // WHERE ID
